...
bool pointcloud_labeling_tool::init(cgv::render::context& ctx)
{
	...
	controller_label_variants[1][InteractionMode::CONFIG][CLP_SIDE].push_back(
		controller_labels[1].add_variant(CLP_SIDE, "apply\nspacing", controller_label_color));
...

	{
		tool_help_label_id[InteractionMode::LABELING][1] =
			text_labels.add_label(
				"[Painter Labeling]\n"
				"Stick <Vertical> [Left Ctrl]: change mode\n"
				"Stick <Horizontal> [Left Ctrl]: change brush size\n"
				"Trigger [Left Ctrl]: toggle inpaint/outpaint\n"
				"Grip [Left Ctrl]: scale point cloud\n"
				"Y button [Left Ctrl]: undo last operation\n"
		...
		int label_move_right_controller = controller_labels[1].add_variant(CLP_GRIP, "grab", controller_label_color);
		...
		controller_label_variants[1][InteractionMode::LABELING][CLP_SIDE].push_back(
			controller_labels[1].add_variant(CLP_SIDE, "instance--", controller_label_color));
		controller_label_variants[0][InteractionMode::LABELING][CLP_SIDE].push_back(
			controller_labels[0].add_variant(CLP_SIDE, "instance\ncolors", controller_label_color));

		// Trigger labels for LABELING mode
		controller_label_variants[1][InteractionMode::LABELING][CLP_TRIGGER].push_back(
			controller_labels[1].add_variant(CLP_TRIGGER, "paint", controller_label_color));
		controller_label_variants[0][InteractionMode::LABELING][CLP_TRIGGER].push_back(
			controller_labels[0].add_variant(CLP_TRIGGER, "in/outpaint", controller_label_color));

		// Size +/- labels for left controller stick horizontal
		controller_label_variants[0][InteractionMode::LABELING][CLP_TRACKPAD_LEFT].push_back(
	...
	{
		tool_help_label_id[InteractionMode::LABELING_2][1] =
			text_labels.add_label(
				"[Sculpt Labeling]\n"
				"Sculpt to isolate, label apply after confirm!\n"
				"Trigger [Left Ctrl]: toggle outcrop/incrop\n"
				"Stick <Vertical> [Left Ctrl]: change mode\n"
				"Stick <Horizontal> [Left Ctrl]: change chisel size\n"
				"Stick Click [Left Ctrl]: cancel sculpt\n"
				"Grip [Left Ctrl]: scale point cloud\n"
				"Y button [Left Ctrl]: undo last label\n"
				"X button [Left Ctrl]: toggle instance colors\n"
				"Trigger [Right Ctrl]: sculpt (hide points)\n"
				"Stick <Vertical> [Right Ctrl]: move forward/back\n"
				"Stick <Horizontal> [Right Ctrl]: change chisel shape\n"
				"Stick Click [Right Ctrl]: confirm label\n"
				"Grip [Right Ctrl]: invert plane / grab\n"
				"B button [Right Ctrl]: instance++\n"
				"A button [Right Ctrl]: instance--\n",
				tool_label_color);

		auto li = tool_help_label_id[InteractionMode::LABELING_2][1];
		text_labels.place_label(li,
			tool_help_label_offset, tool_label_ori, tool_help_label_coordinate_system, LA_LEFT, tool_label_scale);
		text_labels.hide_label(li);

		controller_label_variants[1][InteractionMode::LABELING_2][CLP_TRACKPAD_RIGHT].push_back(
			controller_labels[1].add_variant(CLP_TRACKPAD_RIGHT, "shape->", controller_label_color));

		controller_label_variants[1][InteractionMode::LABELING_2][CLP_TRACKPAD_LEFT].push_back(
			controller_labels[1].add_variant(CLP_TRACKPAD_LEFT, "<-shape", controller_label_color));

		// reserve some space for each pickable shape + paste mode
		controller_label_variants[1][InteractionMode::LABELING_2][CLP_GRIP].resize(selection_shape::NUM_OF_SHAPES + 1, -1);

		// set grip button texts
		int label_move_right_controller_2 = controller_labels[1].add_variant(CLP_GRIP, "grab", controller_label_color);
		for (int v = 0; v < controller_label_variants[1][InteractionMode::LABELING_2][CLP_GRIP].size(); ++v)
			controller_label_variants[1][InteractionMode::LABELING_2][CLP_GRIP][v] = label_move_right_controller_2;

		controller_label_variants[1][InteractionMode::LABELING_2][CLP_GRIP][selection_shape::SS_PLANE] =
			controller_labels[1].add_variant(CLP_GRIP, "invert", controller_label_color);

		controller_label_variants[0][InteractionMode::LABELING_2][CLP_GRIP].push_back(
			controller_labels[0].add_variant(CLP_GRIP, "scale", controller_label_color));

		controller_label_variants[1][InteractionMode::LABELING_2][CLP_MENU_BUTTON].push_back(
			controller_labels[1].add_variant(CLP_MENU_BUTTON, "instance++", controller_label_color));
		controller_label_variants[0][InteractionMode::LABELING_2][CLP_MENU_BUTTON].push_back(
			controller_labels[0].add_variant(CLP_MENU_BUTTON, "undo", controller_label_color));
		controller_label_variants[1][InteractionMode::LABELING_2][CLP_SIDE].push_back(
			controller_labels[1].add_variant(CLP_SIDE, "instance--", controller_label_color));
		controller_label_variants[0][InteractionMode::LABELING_2][CLP_SIDE].push_back(
			controller_labels[0].add_variant(CLP_SIDE, "instance\ncolors", controller_label_color));

		// Stick click labels: cancel (left, red) and confirm (right, green)
		controller_label_variants[0][InteractionMode::LABELING_2][CLP_TRACKPAD_CENTER].push_back(
			controller_labels[0].add_variant(CLP_TRACKPAD_CENTER, "cancel", rgba(0.9f, 0.2f, 0.2f, 1.0f)));
		controller_label_variants[1][InteractionMode::LABELING_2][CLP_TRACKPAD_CENTER].push_back(
			controller_labels[1].add_variant(CLP_TRACKPAD_CENTER, "confirm", rgba(0.2f, 0.9f, 0.2f, 1.0f)));

		// Trigger labels for LABELING_2 (sculpt) mode
		controller_label_variants[1][InteractionMode::LABELING_2][CLP_TRIGGER].push_back(
			controller_labels[1].add_variant(CLP_TRIGGER, "sculpt", controller_label_color));
		controller_label_variants[0][InteractionMode::LABELING_2][CLP_TRIGGER].push_back(
			controller_labels[0].add_variant(CLP_TRIGGER, "in/outcrop", controller_label_color));

		// Size +/- labels for left controller stick horizontal (sculpt chisel size)
		controller_label_variants[0][InteractionMode::LABELING_2][CLP_TRACKPAD_LEFT].push_back(
			controller_labels[0].add_variant(CLP_TRACKPAD_LEFT, "- size", controller_label_color));
		controller_label_variants[0][InteractionMode::LABELING_2][CLP_TRACKPAD_RIGHT].push_back(
			controller_labels[0].add_variant(CLP_TRACKPAD_RIGHT, "size +", controller_label_color));

		// Sculpt mode indicator label (shown on left controller)
		sculpt_mode_label_id = text_labels.add_label(
			"Mode: OUTCROP", rgba(0.9f, 0.6f, 0.1f, 1.0f));
		text_labels.place_label(sculpt_mode_label_id,
	...
	// Painter mode indicator label (shown on left controller)
	{
		painter_mode_label_id = text_labels.add_label(
			"Mode: INPAINT", rgba(0.9f, 0.6f, 0.1f, 1.0f));
	...
	// Trigger label for CONFIG mode (right: measure spacing)
...

	{
		...
		controller_label_variants[1][InteractionMode::TELEPORT][CLP_GRIP].push_back(
			controller_labels[1].add_variant(CLP_GRIP, "grab cloud", controller_label_color));
		controller_label_variants[0][InteractionMode::TELEPORT][CLP_GRIP].push_back(
			controller_labels[0].add_variant(CLP_GRIP, "scale cloud", controller_label_color));
	...
	{
		interaction_mode_label_id = text_labels.add_label(
			"TELEPORT", rgba(0.2f, 0.7f, 0.9f, 1.0f));
	...
	{ // point spacing tool
		spacing_tool_template_str =
			...
			"press the Trigger [Right Ctrl] to measure the point spacing.\n"
...
void pointcloud_labeling_tool::init_frame(cgv::render::context& ctx)
{
	...
	// show/hide sculpt mode indicator based on mode
	if (sculpt_mode_label_id != (uint32_t)-1) {
		if ((InteractionMode)interaction_mode == InteractionMode::LABELING_2)
			text_labels.show_label(sculpt_mode_label_id);
		else
			text_labels.hide_label(sculpt_mode_label_id);
	}

	// show/hide painter mode indicator based on mode
	if (painter_mode_label_id != (uint32_t)-1) {
...
void pointcloud_labeling_tool::build_palette()
{
	...
	*/
	...
	std::function<void(picked_object)> palette_picking_func = [this](picked_object po) {
		// Debounce: ignore rapid successive picks within 200 ms to prevent accidental double-clicks
		static auto s_last_pick = std::chrono::steady_clock::now() - std::chrono::seconds(1);
		auto s_now = std::chrono::steady_clock::now();
		if (std::chrono::duration<double>(s_now - s_last_pick).count() < 0.20)
			return;
		s_last_pick = s_now;
		...
		case vrui::POG_NONE: { // it's one of the 25 spheres from the center
			if (ix < 0 || ix >= num_interactive_palette_slots)
				break;
		...
		case vrui::POG_LEFT_TOOLBAR: {
			// Right-side shortcut buttons, only active in labeling modes
			if (!is_labeling_mode(interaction_mode))
				break;
			int pos = po.position_in_group();

			// Paint mode (LABELING) exclusive buttons: rows 1 and 2
			if ((InteractionMode)interaction_mode == InteractionMode::LABELING) {
				static constexpr float palette_size_step = 0.005f; // larger step than joystick
				if (pos == 0) {
					// +SIZE
					switch (point_selection_shape) {
					case SS_SPHERE:   sphere_style_rhand.radius = std::max(0.005f, sphere_style_rhand.radius + palette_size_step); break;
					case SS_CUBOID:   cube_length = std::max(0.005f, cube_length + palette_size_step); cube_rhand = box3(vec3(-cube_length, -cube_length, -cube_length), vec3(cube_length, cube_length, cube_length)); break;
					case SS_CONE:     cone_height = std::max(0.005f, cone_height + palette_size_step); break;
					case SS_CYLINDER: cylinder_radius = std::max(0.005f, cylinder_radius + palette_size_step); cylinder_height = cylinder_radius * 3.0f; break;
					default: break;
					}
				} else if (pos == 1) {
					// -SIZE
					...
					// IN: set inpaint mode directly
					painter_outpaint_mode = false;
					if (painter_mode_label_id != (uint32_t)-1) {
						text_labels.update_label_text(painter_mode_label_id, "Mode: INPAINT");
						text_labels.place_label(painter_mode_label_id, vec3(-0.06, 0.0, -0.02), tool_label_ori, CS_LEFT_CONTROLLER, LA_RIGHT, tool_label_scale);
					}
					update_controller_labels();
				} else if (pos == 3) {
					// OUT: set outpaint mode directly
					painter_outpaint_mode = true;
					if (painter_mode_label_id != (uint32_t)-1) {
						text_labels.update_label_text(painter_mode_label_id, "Mode: OUTPAINT");
						text_labels.place_label(painter_mode_label_id, vec3(-0.06, 0.0, -0.02), tool_label_ori, CS_LEFT_CONTROLLER, LA_RIGHT, tool_label_scale);
					}
					update_controller_labels();
				}
			...
			// Sculpt mode (LABELING_2) exclusive buttons: rows 1-2 (+SZ/-SZ/IN/OUT) and row 6 (COMMIT/CANCEL)
			if ((InteractionMode)interaction_mode == InteractionMode::LABELING_2) {
				static constexpr float palette_size_step = 0.005f;
				if (pos == 0) {
					// +SIZE
					switch (point_selection_shape) {
					case SS_SPHERE:   sphere_style_rhand.radius = std::max(0.005f, sphere_style_rhand.radius + palette_size_step); break;
					...
					// IN: set incrop mode directly
					sculpt_incrop_mode = true;
					if (sculpt_mode_label_id != (uint32_t)-1) {
						text_labels.update_label_text(sculpt_mode_label_id, "Mode: INCROP");
						text_labels.place_label(sculpt_mode_label_id, vec3(-0.06, 0.0, -0.02), tool_label_ori, CS_LEFT_CONTROLLER, LA_RIGHT, tool_label_scale);
					}
					update_controller_labels();
				} else if (pos == 3) {
					// OUT: set outcrop mode directly
					sculpt_incrop_mode = false;
					if (sculpt_mode_label_id != (uint32_t)-1) {
						text_labels.update_label_text(sculpt_mode_label_id, "Mode: OUTCROP");
						text_labels.place_label(sculpt_mode_label_id, vec3(-0.06, 0.0, -0.02), tool_label_ori, CS_LEFT_CONTROLLER, LA_RIGHT, tool_label_scale);
					}
					update_controller_labels();
				} else if (pos == 10) {
					// COMMIT: confirm sculpt label (mirrors right stick click in LABELING_2)
					if (point_editing_tool == pallete_tool::PT_BRUSH && sculpt_marker_applied) {
						if (!check_instance_constraint()) {
							...
							point_server_ptr->label_all_points(picked_label, (GLint)point_label_group::VISIBLE, 0, point_label_operation::REPLACE);
							point_server_ptr->ref_interaction_settings().enable_history = false;
							GLint restore_visible = make_label(0, (uint32_t)point_label_group::VISIBLE);
							point_server_ptr->label_all_points(restore_visible, SCULPT_MARKER_BIT, 0, point_label_operation::OR);
					...
					// CANCEL: discard sculpt session (mirrors left stick click in LABELING_2)
					if (point_editing_tool == pallete_tool::PT_BRUSH && sculpt_marker_applied) {
						point_server_ptr->ref_interaction_settings() = point_cloud_interaction_settings;
						point_server_ptr->ref_interaction_settings().enable_history = false;
						GLint restore_visible = make_label(0, (uint32_t)point_label_group::VISIBLE);
			...
			// Control Points mode (LABELING_3) exclusive buttons: row 1 (+VAL/PARAM_TOGGLE/-VAL), row 2 (ADD P/DEL P), row 6 (COMMIT/CANCEL)
			if ((InteractionMode)interaction_mode == InteractionMode::LABELING_3) {
				if (pos == 0) {
					// +VALUE: increase selected CP parameter
					switch (cp_selected_param) {
					case 0: cp_Omega_max = std::max(10.f, cp_Omega_max + 10.f); break;
					case 1: cp_alpha_p += 0.1f; break;
					case 2: cp_alpha_n += 0.1f; break;
					...
					switch (cp_selected_param) {
					case 0: cp_Omega_max = std::max(10.f, cp_Omega_max - 10.f); break;
					case 1: cp_alpha_p = std::max(0.f, cp_alpha_p - 0.1f); break;
					case 2: cp_alpha_n = std::max(0.f, cp_alpha_n - 0.1f); break;
					...
					update_cp_params_label(); // also updates palette button label
				} else if (pos == 2) {
					// ADD P: switch to additive CP placement mode
					cp_subtractive_mode = false;
					if (cp_mode_label_id != (uint32_t)-1) {
						text_labels.update_label_text(cp_mode_label_id, "Mode: ADD CP");
						text_labels.place_label(cp_mode_label_id, vec3(-0.06, 0.0, -0.02), tool_label_ori, CS_LEFT_CONTROLLER, LA_RIGHT, tool_label_scale);
					}
					update_controller_labels();
				} else if (pos == 3) {
					// DEL P: switch to subtractive CP deletion mode
					cp_subtractive_mode = true;
					if (cp_mode_label_id != (uint32_t)-1) {
						text_labels.update_label_text(cp_mode_label_id, "Mode: DEL CP");
						text_labels.place_label(cp_mode_label_id, vec3(-0.06, 0.0, -0.02), tool_label_ori, CS_LEFT_CONTROLLER, LA_RIGHT, tool_label_scale);
					}
					update_controller_labels();
				} else if (pos == 10) {
					...
					if (!cp_control_points.empty()) {
						if (!check_instance_constraint()) {
							...
							{
								UndoEntry _e; _e.is_cp = true;
								_e.cpu_labels = std::move(cp_undo_buffer);
								unified_undo_stack.push_back(std::move(_e));
						...
						std::cout << "[CP] Nothing to commit (no control points placed).\n";
					}
				} else if (pos == 11) {
			...
			if (pos == 4) {
				...
				show_instance_colors = false;
				if (instance_counter_label_id != (uint32_t)-1) {
					text_labels.update_label_text(instance_counter_label_id, "Instance: " + std::to_string(instance_counter));
					text_labels.place_label(instance_counter_label_id, left_tool_label_offset, tool_label_ori, CS_LEFT_CONTROLLER, LA_RIGHT, tool_label_scale);
				}
			}
			else if (pos == 6) {
				// INST: switch to instance color view
				show_instance_colors = true;
				if (instance_counter_label_id != (uint32_t)-1) {
					text_labels.update_label_text(instance_counter_label_id, "Instance: " + std::to_string(instance_counter) + " [VIS]");
					text_labels.place_label(instance_counter_label_id, left_tool_label_offset, tool_label_ori, CS_LEFT_CONTROLLER, LA_RIGHT, tool_label_scale);
				}
			}
			else if (pos == 7) {
				// +COUNT: increment instance counter
				instance_counter++;
				recompute_instance_label();
			}
			else if (pos == 9) {
				// -COUNT: decrement instance counter (min 0)
...
bool pointcloud_labeling_tool::handle(cgv::gui::event & e)
{
		...
		case cgv::gui::EID_POSE:
		{
			...
			if (ci == 1) {
				if (state.controller[1].axes[2] > 0.25 || start_l) {
					...
					if ((InteractionMode)interaction_mode == InteractionMode::LABELING_2) {
						// --- SCULPT MODE: hide points with brush ---
						if (point_editing_tool == pallete_tool::PT_BRUSH) {
							cgv::render::context& ctx = *get_context();
							point_server_ptr->ref_interaction_settings() = point_cloud_interaction_settings;
							// Disable history for ALL sculpt operations
							point_server_ptr->ref_interaction_settings().enable_history = false;

							// Apply marker bit to all visible points (once per sculpt session)
							if (!sculpt_marker_applied) {
								sculpt_marker_applied = true;
								GLint marker = make_label(0, (uint32_t)SCULPT_MARKER_BIT);
								point_server_ptr->label_all_points(marker, (GLint)point_label_group::VISIBLE, 0, point_label_operation::OR);
							}

							if (!sculpt_incrop_mode) {
								// OUTCROP: remove VISIBLE bit from points INSIDE brush (continuous)
								GLint hide_label = ~make_label(0, point_label_group::VISIBLE);
								GLint affect_mask = (GLint)point_label_group::VISIBLE;
								GLint exclude_mask = 0;

								switch (point_selection_shape) {
								case SS_SPHERE:
									point_server_ptr->label_points_in_sphere(hide_label, affect_mask, exclude_mask, cgv::math::pose_position(controller_poses[point_selection_hand]) + shape_offset(sphere_style_rhand.radius), sphere_style_rhand.radius, point_label_operation::AND);
									break;
								case SS_CUBOID:
									point_server_ptr->label_points_in_box(hide_label, affect_mask, exclude_mask, cube_rhand, cgv::math::pose_position(controller_poses[point_selection_hand]) + shape_offset(cube_rhand.get_extent().z()*0.5f), cube_orientation_rhand, point_label_operation::AND);
									break;
								case SS_CONE:
									point_server_ptr->label_points_in_cone(hide_label, affect_mask, exclude_mask, cgv::math::pose_position(controller_poses[point_selection_hand]) + shape_offset(0.0f), cone_height, cone_orientation_rhand, point_label_operation::AND);
									break;
								case SS_CYLINDER:
									point_server_ptr->label_points_in_cylinder(hide_label, affect_mask, exclude_mask, cgv::math::pose_position(controller_poses[point_selection_hand]) + shape_offset(cylinder_radius), cylinder_radius, cylinder_height, cylinder_orientation_rhand, point_label_operation::AND);
									break;
								case SS_PLANE:
									point_server_ptr->label_points_by_clipping(hide_label, affect_mask, exclude_mask, cgv::math::pose_position(controller_poses[point_selection_hand]) + curr_offset_rhand, cgv::math::pose_orientation(controller_poses[point_selection_hand]), plane_invert_selection, point_label_operation::AND);
									break;
								}
							}
							else if (!sculpt_incrop_triggered) {
								// INCROP: edge-triggered — hide visible points OUTSIDE brush only
								sculpt_incrop_triggered = true;
								GLint mark_label = make_label(0, (uint32_t)point_label_group::SELECTED_BIT);
								GLint hide_label = ~make_label(0, point_label_group::VISIBLE);
								GLint unmark_label = ~make_label(0, (uint32_t)point_label_group::SELECTED_BIT);

								// Pass 1: Mark visible points INSIDE brush with SELECTED_BIT (protect them)
								switch (point_selection_shape) {
								case SS_SPHERE:
									point_server_ptr->label_points_in_sphere(mark_label, (GLint)point_label_group::VISIBLE, 0, cgv::math::pose_position(controller_poses[point_selection_hand]) + shape_offset(sphere_style_rhand.radius), sphere_style_rhand.radius, point_label_operation::OR);
									break;
								case SS_CUBOID:
									point_server_ptr->label_points_in_box(mark_label, (GLint)point_label_group::VISIBLE, 0, cube_rhand, cgv::math::pose_position(controller_poses[point_selection_hand]) + shape_offset(cube_rhand.get_extent().z()*0.5f), cube_orientation_rhand, point_label_operation::OR);
									break;
								case SS_CONE:
									point_server_ptr->label_points_in_cone(mark_label, (GLint)point_label_group::VISIBLE, 0, cgv::math::pose_position(controller_poses[point_selection_hand]) + shape_offset(0.0f), cone_height, cone_orientation_rhand, point_label_operation::OR);
									break;
								case SS_CYLINDER:
									point_server_ptr->label_points_in_cylinder(mark_label, (GLint)point_label_group::VISIBLE, 0, cgv::math::pose_position(controller_poses[point_selection_hand]) + shape_offset(cylinder_radius), cylinder_radius, cylinder_height, cylinder_orientation_rhand, point_label_operation::OR);
									break;
								case SS_PLANE:
									point_server_ptr->label_points_by_clipping(mark_label, (GLint)point_label_group::VISIBLE, 0, cgv::math::pose_position(controller_poses[point_selection_hand]) + curr_offset_rhand, cgv::math::pose_orientation(controller_poses[point_selection_hand]), plane_invert_selection, point_label_operation::OR);
									break;
								}

								// Pass 2: Hide all VISIBLE points that DON'T have SELECTED_BIT (exclude marked ones)
								point_server_ptr->label_all_points(hide_label, (GLint)point_label_group::VISIBLE, (GLint)point_label_group::SELECTED_BIT, point_label_operation::AND);

								// Pass 3: Remove SELECTED_BIT from all points (cleanup temp marker)
								point_server_ptr->label_all_points(unmark_label, (GLint)point_label_group::GROUP_MASK, 0, point_label_operation::AND);
							}
						}
					}
						...
						if (point_editing_tool == pallete_tool::PT_BRUSH && !check_instance_constraint()) {
							...
							if (!painter_outpaint_mode) {
								// INPAINT: label points INSIDE brush (normal behavior)
								switch (point_selection_shape) {
								case SS_SPHERE:
							...
							else if (!painter_outpaint_triggered) {
								// OUTPAINT: edge-triggered — label all points OUTSIDE brush
								painter_outpaint_triggered = true;
								GLint mark_label = make_label(0, (uint32_t)point_label_group::SELECTED_BIT);
								GLint unmark_label = ~make_label(0, (uint32_t)point_label_group::SELECTED_BIT);

								// Disable history for temp marker passes (only Pass 2 should be undoable)
								bool history_flag = point_cloud_interaction_settings.enable_history;
								point_server_ptr->ref_interaction_settings().enable_history = false;

								// Pass 1: Mark points INSIDE brush with SELECTED_BIT (protect them from labeling)
								switch (point_selection_shape) {
								case SS_SPHERE:
									point_server_ptr->label_points_in_sphere(mark_label, point_selection_group_mask, point_selection_exclude_group_mask, cgv::math::pose_position(controller_poses[point_selection_hand]) + shape_offset(sphere_style_rhand.radius), sphere_style_rhand.radius, point_label_operation::OR);
									break;
								case SS_PLANE:
									point_server_ptr->label_points_by_clipping(mark_label, point_selection_group_mask, point_selection_exclude_group_mask,
										cgv::math::pose_position(controller_poses[point_selection_hand]) + curr_offset_rhand,
										cgv::math::pose_orientation(controller_poses[point_selection_hand]), plane_invert_selection, point_label_operation::OR);
									break;
								case SS_CUBOID:
									point_server_ptr->label_points_in_box(mark_label, point_selection_group_mask, point_selection_exclude_group_mask,
										cube_rhand, cgv::math::pose_position(controller_poses[point_selection_hand]) + shape_offset(cube_rhand.get_extent().z()*0.5f), cube_orientation_rhand, point_label_operation::OR);
									break;
								case SS_CONE:
									point_server_ptr->label_points_in_cone(mark_label, point_selection_group_mask, point_selection_exclude_group_mask,
										cgv::math::pose_position(controller_poses[point_selection_hand]) + shape_offset(0.0f), cone_height, cone_orientation_rhand, point_label_operation::OR);
									break;
								case SS_CYLINDER:
									point_server_ptr->label_points_in_cylinder(mark_label, point_selection_group_mask, point_selection_exclude_group_mask,
										cgv::math::pose_position(controller_poses[point_selection_hand]) + shape_offset(cylinder_radius), cylinder_radius, cylinder_height, cylinder_orientation_rhand, point_label_operation::OR);
									break;
								case SS_ALL:
									// outpaint with SS_ALL makes no sense (nothing is inside → everything outside)
									point_server_ptr->ref_interaction_settings().enable_history = history_flag;
									point_server_ptr->label_all_points(picked_label, point_selection_group_mask, point_selection_exclude_group_mask, picked_label_operation);
									break;
								}

								if (point_selection_shape != SS_ALL) {
									// Pass 2: Label all points that DON'T have SELECTED_BIT (i.e. outside the brush)
									// Re-enable history so this is the only undoable operation
									point_server_ptr->ref_interaction_settings().enable_history = history_flag;
									point_server_ptr->label_all_points(picked_label, point_selection_group_mask,
										point_selection_exclude_group_mask | (GLint)point_label_group::SELECTED_BIT, picked_label_operation);

									// Pass 3: Clear SELECTED_BIT from all points (cleanup temp marker)
									// Disable history again — this is just cleanup, not undoable
									point_server_ptr->ref_interaction_settings().enable_history = false;
									point_server_ptr->label_all_points(unmark_label, (GLint)point_label_group::GROUP_MASK, 0, point_label_operation::AND);
								}
			...
			case vr::VR_A:
				...
				else if (vrke.get_controller_index() == 1)
				{
					if (is_labeling_mode(interaction_mode)) {
						...
						// Position in front of and slightly above the table
						vr_view_ptr->set_tracking_origin(vec3(0.f, 1.2f, 1.0f));
					}
				}
				break;
			...
			case vr::VR_INPUT0:
				...
				if (vrke.get_controller_index() == 0 && (InteractionMode)interaction_mode == InteractionMode::LABELING_2) {
					if (point_editing_tool == pallete_tool::PT_BRUSH && sculpt_marker_applied) {
						...
						// Restore VISIBLE bit on all hidden points using marker
						GLint restore_visible = make_label(0, (uint32_t)point_label_group::VISIBLE);
						point_server_ptr->label_all_points(restore_visible, SCULPT_MARKER_BIT, 0, point_label_operation::OR);

						// Remove SCULPT_MARKER_BIT from all points
						GLint clear_marker = ~make_label(0, (uint32_t)SCULPT_MARKER_BIT);
						point_server_ptr->label_all_points(clear_marker, (GLint)point_label_group::GROUP_MASK, 0, point_label_operation::AND);
				...
				if (vrke.get_controller_index() == 1 && (InteractionMode)interaction_mode == InteractionMode::LABELING_2) {
					if (point_editing_tool == pallete_tool::PT_BRUSH && sculpt_marker_applied) {
						// Check instance constraint before confirming
						if (!check_instance_constraint()) {
						...
						// Step 1: Label all currently VISIBLE points (history ON — records only the subset being labeled)
						point_server_ptr->label_all_points(picked_label, (GLint)point_label_group::VISIBLE, 0, point_label_operation::REPLACE);

						// Step 2: Restore VISIBLE bit on all hidden points (history OFF)
						point_server_ptr->ref_interaction_settings().enable_history = false;
						GLint restore_visible = make_label(0, (uint32_t)point_label_group::VISIBLE);
						point_server_ptr->label_all_points(restore_visible, SCULPT_MARKER_BIT, 0, point_label_operation::OR);

						// Step 3: Remove SCULPT_MARKER_BIT from all points (history OFF)
						GLint clear_marker = ~make_label(0, (uint32_t)SCULPT_MARKER_BIT);
						point_server_ptr->label_all_points(clear_marker, (GLint)point_label_group::GROUP_MASK, 0, point_label_operation::AND);
						...
						// Reset sculpt session state
						sculpt_marker_applied = false;

						std::cout << "Sculpt confirmed: labeled visible points with label " << picked_label
							<< " (instance " << instance_counter << ")" << std::endl;

						// Auto-increment instance counter for next sculpt
						instance_counter++;
						recompute_instance_label();
			...
			case vr::VR_GRIP: 
				{
					...
					case  InteractionMode::TELEPORT:
					{
						// Grip in teleport mode grabs the cloud (handled by is_rotating_moving)
						break;
					}
	...
	if (e.get_kind() == cgv::gui::EID_STICK) {
		...
		if (vrse.get_controller_index() == 1)
		{
			if (vrse.get_action() == cgv::gui::SA_MOVE) {
				...
				// Vertical (up-down) movement: move tracking origin (auto-repeat while held)
				int y_direction = abs(vrse.get_y()) > vertical_threshold ? (cgv::math::sign(vrse.get_y()) >= 0 ? 1 : -1) : 0;
				static auto last_move_time = std::chrono::steady_clock::now();
				static constexpr double move_repeat_interval = 0.15; // seconds between auto-repeat steps
				if (y_direction == 0) {
					...
					if (y_direction != last_stick_y_direction[1] || elapsed >= move_repeat_interval) {
	...
	if (e.get_kind() == cgv::gui::EID_THROTTLE) {
		...
		// Use axes[2] (trigger) directly from state to detect trigger threshold crossings
		// This avoids reacting to grip throttle events
		const vr::vr_kit_state* state = vr_view_ptr->get_current_vr_state();
		if (state) {
			float trigger_value = state->controller[ci].axes[2];
			if (last_throttle_state[ci] < throttle_threshold && trigger_value >= throttle_threshold) {
				on_throttle_threshold(ci, true);
			}
			else if (last_throttle_state[ci] >= throttle_threshold && trigger_value < throttle_threshold) {
				on_throttle_threshold(ci, false);
			}
			last_throttle_state[ci] = trigger_value;
		}
		return true;
	}
	return false;
...
void pointcloud_labeling_tool::on_throttle_threshold(const int ci, const bool low_high) {
	...
	if (ci == 0 && low_high && (InteractionMode)interaction_mode == InteractionMode::LABELING_2) {
		...
		// Update the sculpt mode indicator label
		if (sculpt_mode_label_id != (uint32_t)-1) {
			std::string mode_text = sculpt_incrop_mode ? "Mode: INCROP" : "Mode: OUTCROP";
			text_labels.update_label_text(sculpt_mode_label_id, mode_text);
			text_labels.place_label(sculpt_mode_label_id,
				vec3(-0.06, 0.0, -0.02), tool_label_ori, CS_LEFT_CONTROLLER, LA_RIGHT, tool_label_scale);
		}
		std::cout << "Sculpt mode: " << (sculpt_incrop_mode ? "INCROP" : "OUTCROP") << std::endl;
		update_controller_labels();
		return;
	}

	// Left trigger in LABELING: toggle inpaint/outpaint on press
	if (ci == 0 && low_high && (InteractionMode)interaction_mode == InteractionMode::LABELING) {
		painter_outpaint_mode = !painter_outpaint_mode;
		// Update the painter mode indicator label
		if (painter_mode_label_id != (uint32_t)-1) {
			std::string mode_text = painter_outpaint_mode ? "Mode: OUTPAINT" : "Mode: INPAINT";
			text_labels.update_label_text(painter_mode_label_id, mode_text);
			text_labels.place_label(painter_mode_label_id,
				vec3(-0.06, 0.0, -0.02), tool_label_ori, CS_LEFT_CONTROLLER, LA_RIGHT, tool_label_scale);
		}
		std::cout << "Painter mode: " << (painter_outpaint_mode ? "OUTPAINT" : "INPAINT") << std::endl;
		update_controller_labels();
		return;
	}
	...
	if (!low_high) {
		// Reset incrop/outpaint edge detection flags on trigger release
		if (ci == 1) {
			sculpt_incrop_triggered = false;
...
void pointcloud_labeling_tool::recompute_instance_label() {
	// Special labels (delete/clear with semantic_id <= 0) bypass instance encoding entirely
	if (picked_semantic_id <= 0) {
		// Don't modify picked_label — keep whatever was set by palette selection
		// Just update the instance counter display
		if (instance_counter_label_id != (uint32_t)-1) {
			std::string text = "Instance: " + std::to_string(instance_counter);
			if (show_instance_colors) text += " [VIS]";
			text_labels.update_label_text(instance_counter_label_id, text);
		...
		{
			int disp_id = palette.get_instance_display_id();
...
void pointcloud_labeling_tool::update_cp_params_label() {
	...
	// Also update the palette param-toggle button (pos 12) if it exists
	{
		int id = palette.get_shortcut_id(12);
		if (palette.object_id_is_valid(id)) {
			char buf[48];
			if (cp_selected_param == 0)
				std::snprintf(buf, sizeof(buf), "%d/4\n%s:\n%d", cp_selected_param + 1, names[cp_selected_param], (int)cp_Omega_max);
			else {
				float val = (cp_selected_param == 1) ? cp_alpha_p : (cp_selected_param == 2) ? cp_alpha_n : cp_alpha_c;
				std::snprintf(buf, sizeof(buf), "%d/4\n%s:\n%.2f", cp_selected_param + 1, names[cp_selected_param], val);
			}
			palette.set_label_text(id, buf);
...
void pointcloud_labeling_tool::rollback_last_operation(cgv::render::context& ctx) {
	...
	if (is_labeling_mode(interaction_mode) && point_editing_tool == pallete_tool::PT_BRUSH) {
...
void pointcloud_labeling_tool::update_interaction_mode(const InteractionMode im) {
	...
	if (interaction_mode == (int)InteractionMode::LABELING_2 && im != (InteractionMode)interaction_mode && sculpt_marker_applied) {
		...
		GLint restore_label = make_label(0, (uint32_t)point_label_group::VISIBLE);
		point_server_ptr->label_all_points(restore_label, SCULPT_MARKER_BIT, 0, point_label_operation::OR);
		// Remove marker bit from all points
		GLint clear_marker = ~make_label(0, (uint32_t)SCULPT_MARKER_BIT);
		point_server_ptr->label_all_points(clear_marker, (GLint)point_label_group::GROUP_MASK, 0, point_label_operation::AND);
	...
	// Update mode-exclusive shortcut button labels and visibility
	{
		const bool in_paint  = (im == InteractionMode::LABELING);
		...
		// Row 2 pos 3: OUT (paint/sculpt), DEL P (CP)
		const char* row_labels[4] = {
			in_cp ? "+VAL" : (show_size_inout ? "+SZ"  : ""),
			in_cp ? "-VAL" : (show_size_inout ? "-SZ"  : ""),
			in_cp ? "ADD P": (show_size_inout ? "IN"   : ""),
			in_cp ? "DEL P": (show_size_inout ? "OUT"  : "")
		};
		for (int p = 0; p < 4; ++p) {
		...
		{
			int id = palette.get_shortcut_id(12);
			palette.set_object_visibility(id, in_cp);
			if (in_cp && palette.object_id_is_valid(id)) {
				static const char* pnames[4] = { "Omega", "alpha_p", "alpha_n", "alpha_c" };
				char buf[48];
				if (cp_selected_param == 0)
					std::snprintf(buf, sizeof(buf), "%d/4\n%s: %d", cp_selected_param + 1, pnames[cp_selected_param], (int)cp_Omega_max);
				else {
					float val = (cp_selected_param == 1) ? cp_alpha_p : (cp_selected_param == 2) ? cp_alpha_n : cp_alpha_c;
					std::snprintf(buf, sizeof(buf), "%d/4\n%s: %.2f", cp_selected_param + 1, pnames[cp_selected_param], val);
				}
				palette.set_label_text(id, buf); // set_label_text also calls show_label internally
			} else {
		...
		{
			const bool show_row6 = !in_paint;
			const bool row6_active = (in_sculpt || in_cp);
			const char* r6_labels[2] = { row6_active ? "COMMIT" : "", row6_active ? "CANCEL" : "" };
			for (int i = 0; i < 2; ++i) {
				int id = palette.get_shortcut_id(10 + i);
				palette.set_object_visibility(id, show_row6);
				if (!show_row6) {
					palette.hide_object_label(id); // prevent empty-string baseline from showing in paint mode
				} else if (row6_active && palette.object_id_is_valid(id)) {
					palette.set_label_text(id, r6_labels[i]); // set_label_text also calls show_label
				} else {
					palette.hide_object_label(id); // visible but no active label (other labeling modes)
				}
			}
		}
		palette.set_palette_changed();
...
bool pointcloud_labeling_tool::teleport_ray(const vec3& direction, const vec3& origin, const float ray_radius)
{
	...
	if (point_server_ptr->raycast_points(direction, origin, ray_radius, t, p)) {
		// Just exit overview mode without restoring the cloud transformation
		overview_mode = false;

		vec3 new_position = vr_view_ptr->get_tracking_origin() + (p - origin);
		vr_view_ptr->set_tracking_origin(new_position);
